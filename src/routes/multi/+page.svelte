<script>
  ...
  onMount(async () => {
    ...
    ARnft.ARnft.initWithEntities(width, height, entities, "config.json", true)
      .then((nft) => {
        ...
        document.addEventListener("containerEvent", function (ev) {
          ...
          window.addEventListener(
            "getMatrixGL_RH-" + nft.uuid + "-pinball",
            function () {
              console.log("this is pinball!!");
            }
          );
          window.addEventListener(
            "getMatrixGL_RH-" + nft.uuid + "-kuva",
            function () {
              console.log("this is kuva!!");
            }
          );
          window.addEventListener(
            "getMatrixGL_RH-" + nft.uuid + "-chalk_multi",
            function () {
